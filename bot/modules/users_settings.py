...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'metadata': ['Videos Metadata Text Editor.', 'Send Title Text\n<b>Timeout:</b> 60 sec'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        text = BotTheme('LEECH', NAME=name, DL=f"{dailyll} / {dailytlle}",
                ...
                LSUFFIX=escape(lsuffix), LDUMP=ldump, METADATA=escape(metadata), LREMNAME=escape(lremname))
        ...
        elif key in ['lprefix', 'metadata', 'lremname', 'lsuffix', 'lcaption', 'ldump']:
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'metadata', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'yt_opt'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['lprefix', 'metadata', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'mprefix', 'msuffix', 'mremname']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
        return_key = 'leech' if data[2][0] == 'l' or data[2] == 'metadata' else 'mirror'
        await update_user_settings(query, data[2], return_key, edit_mode)
        if not edit_mode: return
        pfunc = partial(set_custom, pre_event=query, key=data[2])
        rfunc = partial(update_user_settings, query, data[2], return_key)
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlsuffix', 'dlremname', 'dlcaption', 'dldump', 'dlmetadata']:
        handler_dict[user_id] = False
        await query.answer()
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['dmprefix', 'dmsuffix', 'dmremname', 'dmetadata', 'duser_tds']:
        handler_dict[user_id] = False
        await query.answer()
