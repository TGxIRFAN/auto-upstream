...
from bot import LOGGER, MAX_SPLIT_SIZE, config_dict, user_data, DOWNLOAD_DIR
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    if lcaption and dirpath and not isMirror:
        ...
        prefile_ = re_sub(r'www\S+', '', prefile_).replace('- ', '', 1).replace(r'^www\S+\s*[-_]*\s, '', 1).replace('- ', '').replace('[', '').replace(']', '').replace('&', '').replace('+', '').replace('-', '').replace('-', '')
